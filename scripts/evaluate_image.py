"""
Offline evaluation script for the Image Detection Pipeline.
Executes inference on real labeled test samples and saves evaluation metrics and figures.
"""
import json
import os
import sys
...
logger = get_logger("authentica.evaluate_image")


def run_image_evaluation():
    manifest_path = BASE_DIR / "data" / "test" / "image" / "manifest.json"
    if not manifest_path.exists():
        logger.error(f"Image test manifest not found at {manifest_path}. Please run create_test_images.py first.")
        return

    with open(manifest_path, "r", encoding="utf-8") as f:
        samples = json.load(f)

    logger.info(f"Loaded {len(samples)} image evaluation samples from manifest.")

    detector = ImageDetector()
    try:
        detector.load_model()
    except Exception as e:
        logger.warning(f"Could not load Hugging Face model online ({e}). Falling back to feature artifact analysis for evaluation.")

    y_true = []
    y_scores = []
    metadata = {
        "dataset_name": "Authentica Sanity Image Benchmark",
    ...
    print("\n" + "="*60)
    print("           IMAGE DETECTOR EVALUATION REPORT")
    print("="*60)
